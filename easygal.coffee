...
$ ->
          ...
          $('.fancybox').fancybox
            ...
            afterLoad: ->
              $.extend this,
                ...
                jwidth: '100%'
                jheight: '100%'
                jcontent: '<div class="fancybox-image" style="background-image:url(' + @href + '); background-size: cover; background-position:50% 50%;background-repeat:no-repeat;height:100%;width:100%;" /></div>'
            afterShow: ->
            ...
            helpers:
              title: type: 'inside'
              buttons: {}
              overlay: css: 'background': 'rgba(0, 0, 0, 0.95)'
              title: type: 'over'
  ...
  clickDeleteCheckbox = ->
    if $(this).is(':checked')
      ...
      index = g_imagesToDelete.indexOf($(this).attr('id'))
      if index > -1
  ...
  enableDelete = (enable) ->
    ...
    g_imagesToDelete = []
    updateDeleteConfirmButton()
    $('.gallery-image-right').each ->
      if $(this).find('.glyphicon-ok').length
        $(this).parent('div.row').remove()
        return

      $(this).find('.glyphicon-warning-sign').each ->
        $(this).find('.glyphicon-warning-sign').remove()

      if enable
        $(this).append '<div class="delete-confirmation checkbox"><label>' + '<input type="checkbox" value="0">' + '<i class="glyphicon glyphicon-remove"></i></label></div>'
        $(this).find('input').each ->
          $(this).click clickDeleteCheckbox
      else
        $(this).find('div.delete-confirmation').each ->
          $(this).remove()
  ...
  confirmDelete = ->
    $('div.gallery-image-right').each ->
      url = null
      _this = $(@)

      if _this.find('input').is(':checked')
        _this.find('div.delete-confirmation').each ->
          $(this).remove()
  ...
  $('#gallery-select').change ->
    ...
    index = $('#gallery-select').prop('selectedIndex')
    if index == 0
      gallery.val ''
  ...
  $('input#upload-gallery').on 'change keyup paste', ->
    handleGalleryChange()
  ...
  $('#fileupload').fileupload(
    ...
    stop: (e, data) ->
      ...
      if $('ul.dropdown-menu:has(li #' + gallery + ')').length == 0
        $('ul.dropdown-menu').append '<li class="dropdown-button"><a class="nav-button" id="' + gallery + '" href="#">' + gallery + '</a></li>'
      $(document).click hideOverlays
      navigate gallery
    done: (e, data) ->
